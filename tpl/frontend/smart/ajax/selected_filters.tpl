...
<{if $arItem.selected}>
<a class="filter-element" href="<{$arItem.url}>"><{$arItem.title}></a>
<{/if}>
<{/foreach}>
<{* fixed price filter type *}>
<{elseif $filter.type=='price'}>
<{if $filter.children.selected.min && $filter.children.selected.max}>
<a class="filter-element" href="<{$filter.children.selected.url}>">от <{$filter.children.selected.min}> до <{$filter.children.selected.max}> грн</a>
<{/if}>
<{*range filter type*}>
...
<{/if}>
<{/foreach}>
<a class="filter-element clear-all" href="<{$UrlWL->copy()->resetPage()->resetFilters()->buildUrl()}>"><{$smarty.const.REMOVE_ALL_FILTERS}></a>
<{/if}>