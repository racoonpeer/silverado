...
if ($itemID and $item = getSimpleItemRow($itemID, CATALOG_TABLE) and !empty($item)) {
    ...
    if ($IS_AJAX) {
        ...
        if (isset($_POST["comment"])) {
            ...
            $Validator->validateGeneral($_POST["descr"], "¬ведите текст отзыва", "descr");
            $Validator->validateGeneral($_POST["title"], "¬ведите им€", "title");
            $Validator->validateEmail($_POST["email"], "¬ведите им€", "email");
            if ($Validator->foundErrors()) {
                ...
                if ($DB->postToDB($arPostData, COMMENTS_TABLE)) {
                    $messages[] = "—пасибо за ваш отзыв!";
                    $messages[] = "¬аш отзыв успешно добавлен и будет опубликован после модерации!";
                    $result = "success";
                }
            }
