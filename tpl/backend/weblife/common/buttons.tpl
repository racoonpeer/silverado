...
<div class="buttons_list">
    ...
    <input class="buttons" name="submit_apply" type="submit" value="<{$smarty.const.BUTTON_APPLY}>" onclick="applySubmit(this);"/>
<{if $item.id && $arrPageData.module!='comments'}>
    <input class="buttons" name="submit_copy" type="submit" value="<{$smarty.const.BUTTON_COPY}>" onclick="return userConfirm('copy', '<{$smarty.const.CONFIRM_COPY}>');"/>
<{/if}>
...
<{if $task=='editItem' && $item.id > $deleteIDLimit && (!isset($item.module) || (array_key_exists($item.module, $arrModules) && $item.id != $arrModules[$item.module].id))}>
    <input class="buttons" name="submit_delete" type="submit" value="<{$smarty.const.BUTTON_DELETE}>" onclick="return userConfirm('delete', '<{$smarty.const.CONFIRM_DELETE}>');"/>
<{/if}>
</div>
<script type="text/javascript">
    function applySubmit(btn){
        $(btn).closest("form").prop("action", window.location.href);
    };
