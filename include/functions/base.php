        ...
        if ($iname && $itmp_name) {
            ...
            if (in_array($file_ext, $arExtAllowed)) {
                ...
                if (copy($itmp_name, $idir . $new_name)) {
                    if (($iw != $itmp_w || $ih != $itmp_h) && !createThumb($itmp_name, $iw, $ih, $idir . $new_name)) {
                        return false;
                    }
                    while ($params && (list(, list($partiname, $piw, $pih)) = each($params))) {
    ...
    if (!empty($_FILES[$colname])) {
        ...
        if ($iname && $itmp_name) {
            ...
            if (in_array($file_ext, $arExtAllowed)) {
                ...
                if (!createThumbWithBg($itmp_name, $idir . $new_name, $iw, $ih, $transparent, $bgRGB, $quality)) {
                    return false;
                }
        ...
        if ($iname && $itmp_name) {
            ...
            if (in_array($file_ext, $arExtAllowed)) {
                ...
                if (move_uploaded_file($itmp_name, $idir . $new_name)) {
                    if (($iw != $itmp_w || $ih != $itmp_h) && !createThumb($idir . $new_name, $iw, $ih, $idir . $new_name)) {
                        return false;
                    }
                    while ($params && (list(, list($partiname, $piw, $pih)) = each($params))) {
    ...
    if (is_file($srcfile)) {
        ...
        if ($iname) {
            ...
            if (rename($srcfile, $idir . $destname)) {
                if (($iw != $itmp_w || $ih != $itmp_h) && !createThumb($idir . $destname, $iw, $ih, $idir . $destname)) {
                    return false;
                }
                while ($params && (list(, list($partiname, $piw, $pih)) = each($params))) {
...
function fileUpload_addToDB($fileKey, $id, $table, $colname, $conditions = '', $dir = '', $arExtAllowed = array(), $deleteFirst = false, $toAllLangs = false, $checkToDelete = true)
{
    if ($deleteFirst) {
        if ($toAllLangs) {
            deleteFileFromDB_AllLangs($id, $table, $colname, $conditions, $dir);
        } else {
...
function funcWithAccessLevelMode($func, $path, $mode)
{
    if (function_exists($func) && !empty($path) && !empty($mode)) {
        switch ($mode) {
            case '0644' :
                return @$func($path, 0644);
                break;
            case '0665' :
                return @$func($path, 0665);
                break;
            case '0666' :
                return @$func($path, 0666);
                break;
            case '0755' :
                return @$func($path, 0755);
                break;
            case '0775' :
                return @$func($path, 0775);
                break;
            case '0777' :
                return @$func($path, 0777);
                break;
        }
    }
    return false;
...
function getItemsCountInCategories($idkeyname, $valuename, $table, $selcolnames = '', $conditions = '')
{
    ...
    $result = mysql_query($query);
    while ($row = mysql_fetch_assoc($result)) {
        $items[intval($row[$idkeyname])] = $row[$valuename];
    }
    return $items;
}
...
function getArrValueFromDB($table, $selectcol, $whereoptions = '', $colname = '')
{
    ...
    $result = mysql_query($query);
    while ($row = mysql_fetch_assoc($result)) {
        $items[] = $row[((empty($colname)) ? $selectcol : $colname)];
    }
    return $items;
}
...
function getRowItemsInKeyValue($keyname, $valuename, $table, $selcolnames = '', $whereoptions = '', $orderConditions = '', $limitConditions = '')
{
    ...
    $result = mysql_query($query);
    while ($row = mysql_fetch_assoc($result)) {
        $items[$row[$keyname]] = $row[$valuename];
    }
    return $items;
}
...
function getRowItemsValue($table, $selcolname, $whereoptions = '', $orderConditions = '', $limitConditions = '', $valuename = '')
{
    ...
    $result = mysql_query($query);
    while ($row = mysql_fetch_assoc($result)) {
        $items[] = $row[(empty($valuename) ? $selcolname : $valuename)];
    }
    return $items;
}
...
{
    $items = array();
    $query = "SELECT DISTINCT t.* FROM `" . MAIN_TABLE . "` t
                 WHERE CHAR_LENGTH(t.`module`) > 0
...
function getSqlListFilterField($key, $value, $type, $prefix = '')
{
    switch ($type) {
        ...
        case '<>':
            ...
            break;
        case 'IN':
            return "{$prefix}`{$key}` IN ($value)";
            break;
        case 'BEETWEEN':
            return "{$prefix}`{$key}` BEETWEEN ($value)";
            break;
        default:
            break;
    }
