<script type="text/javascript">
    $(document).ready(function() {
        ...
        var scriptURL = '/interactive/ajax.php?zone=admin&action=ajaxCatalogFilesUpload&<{$smarty.const.WLCMS_SESSION_NAME}>=<{$sessionID}>&PID=<{$arrPageData.pid}>&module=<{$arrPageData.pmodule}>&file_prefix=<{$arrPageData.userfileprefix}>';
        $('#userFile').uploadify({
            'queueID'        : 'fileQueue',
            'uploader'       : '/js/jquery/uploadify/uploadify.swf',
            'script'         : encodeURIComponent(scriptURL+'&uploadifyData=1&files_params=<{$arrPageData.images_params|serialize|base64_encode|urlencode}>'),
            'checkScript'    : encodeURIComponent(scriptURL+'&uploadifyCheck=0'),
            ...
            <td valign="top" >
                <table width="100%" border="0" cellspacing="1" cellpadding="0" class="list" id="operationTbl">
                    <tr>
                        ...
                        <td id="headb" align="center" width="22">По умолчанию</td>
                        <td id="headb" align="left"><{$smarty.const.HEAD_NAME}></td>
                        <td id="headb" align="center" width="22"></td>
...
<{if $items[i].isdefault==1}>
                                <img src="<{$arrPageData.system_images}>check.png" alt="Default" title="Default" />
<{else}>
                            <a href="<{$arrPageData.current_url|cat:$arrPageData.filter_url|cat:"&task=setDefaultItem&itemID="|cat:$items[i].id}>" title="Set default">
                                <img src="<{$arrPageData.system_images}>un_check.png" alt="Set default" title="Set default" />
                            </a>
<{/if}>                        
                        </td>
                        <td >
                            <input type="text" class="field" name="arTitle[<{$items[i].id}>]" value="<{$items[i].title}>" />
                        </td>
                        <td  align="center">
                            <a href="<{$arrPageData.files_url|cat:$items[i].filename}>" onclick="return parent.hs.expand (this, { })" class="highslide">
                                <img src="<{$arrPageData.system_images}>find.png" alt="View" title="View" width="24" />
                            </a>
                        </td>
                        <td  align="center">
...
<{/section}>
                ...
                <table width="100%" border="0" cellspacing="1" cellpadding="0" class="list">
                    <tr>
                        ...
                        <input style="float:right;" type="submit" value="Применить сортировку" class="buttons"/>
                    </tr>
